...
describe('march', () => {
   ...
   test('test small grid', () => {
      ...
      for (let pos=0; pos<cubes[0].vertices.length; pos+=3) {
         const vert = cubes[0].vertices.slice(pos, pos+3);
         expect(
            Math.abs(vert[0]) === 0.5 || 
            Math.abs(vert[1]) === 0.5 || 
            Math.abs(vert[2]) === 0.5
         ...
         expect(Math.abs(vert[0]) === 0.5 && Math.abs(vert[1]) === 0.5).toBeFalsy();
         expect(Math.abs(vert[1]) === 0.5 && Math.abs(vert[2]) === 0.5).toBeFalsy();
         expect(Math.abs(vert[2]) === 0.5 && Math.abs(vert[0]) === 0.5).toBeFalsy();
      }
   });
   ...
   test('test inverse grid', () => {
      ...
      //       / |   ->    / |
      //     1 --3       1 --2
      for (let pos=0; pos<cubes1[0].vertices.length; pos+=9) {
         const tri1 = cubes1[0].vertices.slice(pos, pos+9);
         const tri2 = cubes2[0].vertices.slice(pos, pos+9);
   ...
   test('ensure correct sizing of split objects', () => {
      ...
      for (const cube of cubes.slice(0,-1)) {
      ...
      expect(cubes[cubes.length-1].vertices.length).toBeLessThan(MAX_INDICES*3);
   });
});
