...
const buildAxes = (length) => {
   if (
      ...
      throw new Error('buildAxes length argument invalid');
...
const buildPoints = (axis) => {
   if (!axis || !axis.length || typeof axis !== 'object')
      throw new Error('buildPoints axis argument is invalid');
   if (axis.length < 2)
      throw new Error('buildPoints axis argument is too small');
   if (axis.length > 256)
      throw new Error('buildPoints axis argument is too large');
   axis.forEach((n) => {
      if (typeof n !== 'number')
         throw new Error('buildPoints axis argument is invalid');
   });
